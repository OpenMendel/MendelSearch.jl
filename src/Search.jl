"""
Parameter data structure. The various entries control optimization.
"""
module SearchSetup
...
"""
Provides keyword defaults prior to parameter initialization.
"""
function optimization_keywords!(keyword::Dict{ASCIIString, Any})
  ...
  keyword["goal"] = "minimize" # or "maximize"
...
"""
Houses the Search optimization algorithm.
"""
module Search
...
secant update unless an exact value is supplied by the user. Users must 
...
function optimize(fun::Function, parameter::Parameter)
  ...
  #
  error = check_constraints(constraint, level, pname, par, pmin, pmax,
    io, travel)
...
"""
Controls the computation of f and its first two differentials.
When the first partials are computed numerically, forward or central 
differences are used depending on the logical variable forward. The 
numerical differentiation interval is adjusted to take into account 
the magnitude of a parameter and whether it lies on its upper bound.
"""
